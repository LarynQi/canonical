...
app = Flask(__name__)
app.config['FILE_UPLOADS'] = '/Users/lqi/recruiting/canonical/summer-break/db/'

counter = max(map(lambda filename: int(filename.split('.')[0]), filter(lambda filename: filename[-4:] == '.csv', os.listdir(app.config['FILE_UPLOADS'])))) + 1
...
def report():
    ...
    hash_certificate = request.args['password']
    m = hashlib.sha512()
    m.update(open(filepath).read().encode('utf-8'))
